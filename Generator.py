...
from Element import Element, getNominalStiffness
class Generator:
...
def assigndegreesOfFreedom(node):
    dof = np.linspace(0,len(node)*3 - 1, len(node)*3, dtype=int)
    for i in range(len(node)):
        node[i].dof = [dof[3 * i], dof[3 * i + 1], dof[3 * i + 2]]
    return dof
...
def assignElementDofs(self):
    elementDofs = np.zeros((len(self.element), 6), dtype =int)
...
def setBoundaryConditions(self):
    ...
    for node in self.node:
        ...
        if node.thetaFixed == True:
           fixedDofs =  np.append(fixedDofs,node.dof[2])
...
def getElementStiffness(self, xPhys):
    for ele in self.element:
        A = self.Amin+(xPhys[ele.tag])**self.pen*(self.Amax-self.Amin)
        ele.stiffness = getNominalStiffness(ele.length, ele.cos, ele.sin, ele.E, A)